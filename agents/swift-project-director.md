...
- **LSP (`swift-lsp` plugin / SourceKit-LSP)** — check `which sourcekit-lsp` and that the `swift-lsp` plugin is enabled; if the LSP tool call fails, code-intelligence lookups (symbol/reference lookups across the project) fall back to grep/read, which is slower and less precise — flag this in `agent_notes.md` so Implementer/Quality Auditor subagents know why symbol lookups may be manual.
